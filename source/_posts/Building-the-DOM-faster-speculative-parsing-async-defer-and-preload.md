...
对每一个web开发者来说，理解浏览器内部运行机制都是最强大的开发工具。我们将考虑浏览器是如何解析你的代码以及怎样通过预解析更快的加载页面，分解`defer`(延迟)和`async`(异步)是怎样工作的，以及你怎样利用新的关键词`preload`(预加载)。
...
注：`async`和`defer`属性只作用于外部脚本，如果没有`src`，它们会被忽略。
